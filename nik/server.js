...
function placeOrder() {
    ...
    const orderId = ++state.orderCounter;
    const items = Object.values(state.cart);
...
function showReceipt(order) {
    ...
    document.getElementById('modalContent').innerHTML = `
        ...
        <div class="receipt-box">
            <div class="r-row">
                <span><i class="fas fa-hashtag"></i> Order #${order.id}</span>
...
function handleQrScan(decodedText) {
    ...
    if (!match) {
        ...
        messageEl.textContent = 'Invalid QR content. Must include order number.';
        return;
    }
    ...
    if (!order) {
        messageEl.style.display = 'block';
        messageEl.textContent = `Order #${orderId} not found.`;
        return;
    }

    if (order.status === ORDER_STATUS.DELIVERED) {
        messageEl.style.display = 'block';
        messageEl.textContent = `Order #${orderId} already delivered.`;
        return;
    }

    order.status = ORDER_STATUS.DELIVERED;
    messageEl.style.display = 'block';
    messageEl.textContent = `Order #${orderId} marked Delivered.`;
    showToast(`Order #${orderId} updated to Delivered`, 'success');
    renderOrders();
}
    ...
    list.innerHTML = state.orders.map(o => `
        <div class="order-card">
            ...
            <div class="order-items-list">
                ${o.items.map(e => `${e.item.emoji} ${e.item.name} ×${e.qty}`).join(' · ')}
            </div>
...
function init() {
    // Load saved data
    if (state.orders.length) {
        state.orderCounter = Math.max(...state.orders.map(o => o.id)) + 1;
    }
